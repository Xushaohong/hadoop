...
public class CompletedJob implements org.apache.hadoop.mapreduce.v2.app.job.Job {
  ...
  private AtomicBoolean tasksLoaded = new AtomicBoolean(false);
  ...
  public CompletedJob(Configuration conf, JobId jobId, Path historyFile, 
      boolean loadTasks, String userName, Path confFile,
      JobACLsManager aclsMgr) 
          throws IOException {
    if(LOG.isDebugEnabled()) {
      LOG.debug("Loading job: " + jobId + " from file: " + historyFile);
    }
    this.conf = conf;
    this.jobId = jobId;
    this.user = userName;
  ...
  protected synchronized void loadFullHistoryData(boolean loadTasks,
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug("Loading history file: [" + historyFileAbsolute + "]");
    }
  ...
  public List<AMInfo> getAMInfos() {
    ...
    for (JobHistoryParser.AMInfo jhAmInfo : jobInfo
        .getAMInfos()) {
      AMInfo amInfo =
