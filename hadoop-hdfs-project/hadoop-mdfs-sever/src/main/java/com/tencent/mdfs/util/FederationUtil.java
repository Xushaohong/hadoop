...
public final class FederationUtil {
  ...
  public static AbstractDelegationTokenSecretManager<DelegationTokenIdentifier>
    ...
    try {
      // Constructor with configuration but no context
      Constructor<? extends AbstractDelegationTokenSecretManager> constructor = clazz.getConstructor(
          Configuration.class);
      return constructor.newInstance(conf);
    }catch (Exception e){
      LOG.error("new Serectmanager failed",e);
    }
    return null;//newInstance(conf, null, null, clazz);
  }
