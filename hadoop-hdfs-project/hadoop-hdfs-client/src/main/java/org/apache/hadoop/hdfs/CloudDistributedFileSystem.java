...
import static org.apache.hadoop.hdfs.client.HdfsClientConfigKeys.MDFS_CLIENT_RBF_ENABLED_DEFAULT;
import static org.apache.hadoop.hdfs.client.HdfsClientConfigKeys.MDFS_CLIENT_RBF_ENABLED_KEY;
...
public class CloudDistributedFileSystem extends FileSystem
    implements KeyProviderTokenIssuer {
  public static final String MDFS_URI_SCHEME = "mdfs";
  public static final Logger LOG = LoggerFactory.getLogger(CloudDistributedFileSystem.class);
  ...
  DFSClient dfs;
  boolean rbfEnabled = false;
  private boolean verifyChecksum = true;

  private DFSOpsCountStatistics storageStatistics;

  private static String PCG_CLOUD_MDFS = "PCG_CLOUD_MDFS";

  static{
    HdfsConfiguration.init();
  }

  public CloudDistributedFileSystem() {
  }
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    rbfEnabled= conf.getBoolean(MDFS_CLIENT_RBF_ENABLED_KEY,
        MDFS_CLIENT_RBF_ENABLED_DEFAULT);
    URI tmpUri = uri;
    if (rbfEnabled) {
      try {
        // To load hdfs configuration from zk.
        tmpUri = new URIBuilder(uri).setScheme("hdfs").build();
      } catch (URISyntaxException e) {
        LOG.error("Invalid RBF URI: " + e);
      }
    }
    conf = DistributedConfigHelper.get().build(tmpUri, distributedCfg);
    ...
    try {
      if (rbfEnabled) {
        this.dfs = new DFSClient(new URIBuilder(uri).setScheme("hdfs").build(),
            conf, statistics);
      } else {
        URI uriHolder = URI.create("hdfs://"+ conf.get(HdfsClientConfigKeys.MDFS_VNN_ADDRESS));
        conf.setBoolean(PCG_CLOUD_MDFS, true);
        this.dfs = new DFSClient(uriHolder,null, conf, statistics);
      }
    } catch (Exception e) {
      LOG.error(" initialize error "+e);
      throw new RuntimeException("CloudHdfsFileSystem initialize error");
    }
  ...
  String getPathName(Path file) {
    ...
    if(result.startsWith(MDFS_URI_SCHEME)){
      if (rbfEnabled) {
        result = file.toUri().getPath();
      } else {
        result = result.substring(MDFS_URI_SCHEME.length()+":/".length());
      }
    }
    LOG.debug("getPathName "+file+" "+result);
    return result;
  }
  ...
  public boolean recoverLease(final Path f) throws IOException {
    ...
    return new FileSystemLinkResolver<Boolean>() {
      ...
      public Boolean next(final FileSystem fs, final Path p)
          throws IOException {
        if (fs instanceof CloudDistributedFileSystem) {
          CloudDistributedFileSystem myDfs = (CloudDistributedFileSystem)fs;
  ...
  public void unsetStoragePolicy(final Path src)
    ...
    new FileSystemLinkResolver<Void>() {
      ...
      public Void next(final FileSystem fs, final Path p) throws IOException {
        if (fs instanceof CloudDistributedFileSystem) {
          ((CloudDistributedFileSystem) fs).unsetStoragePolicy(p);
          return null;
        } else {
  ...
  protected RemoteIterator<LocatedFileStatus> listLocatedStatus(final Path p,
    ...
    return new FileSystemLinkResolver<RemoteIterator<LocatedFileStatus>>() {
      ...
      public RemoteIterator<LocatedFileStatus> next(final FileSystem fs,
        ...
        if (fs instanceof CloudDistributedFileSystem) {
          return ((CloudDistributedFileSystem)fs).listLocatedStatus(p, filter);
        }
  ...
  public RemoteIterator<FileStatus> listStatusIterator(final Path p)
    ...
    return new FileSystemLinkResolver<RemoteIterator<FileStatus>>() {
      ...
      public RemoteIterator<FileStatus> next(final FileSystem fs, final Path p)
          throws IOException {
        return ((CloudDistributedFileSystem)fs).listStatusIterator(p);
      }
    }.resolve(this, absF);
  ...
  /** @deprecated Use {@link FsStatus} instead */
  @InterfaceAudience.Private
  @Deprecated
   ...
   * @deprecated Use {@link FileSystem#getStatus()}
   * instead */
   @Deprecated
